...
CREATE TABLE IF NOT EXISTS `geodis-oms`.`sale_order` (
  ...
  `status` VARCHAR(45) NULL DEFAULT 'Created' COMMENT 'Statues: Created, Started, Picked/Packed, Completed, Cancelled',
  `connector_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `geodis-oms`.`users` (
  ...
  `status` VARCHAR(45) NULL DEFAULT 'Active' COMMENT 'Statuses: Active, Locked, Expired',
  `created_at` TIMESTAMP NULL,
  `updated_at` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `geodis-oms`.`role` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `role_name` VARCHAR(15) NOT NULL DEFAULT 'User' COMMENT 'Roles: User, Admin',
